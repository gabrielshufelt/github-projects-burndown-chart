...
| Variable | Meaning |
|----------|---------|
| `chart_end_date` | (OPTIONAL) The last day to show on the burndown chart formatted as `YYYY-MM-DD`. <br/><br/> Used to extend the chart beyond the sprint end date without affecting the slope of the ideal burndown line (e.g. to show tasks that were completed after the official end of a sprint). <br/><br/> Example: `2021-10-24` |
| `points_label` | (OPTIONAL) Set to any non-null value to use the Estimate field from GitHub Projects V2 for story points. If set to `null`, the burndown chart will count open issues instead of points. |
